...
let
  ...
  unwrapped = stdenv.mkDerivation rec {
    ...
    version = "7.1.6.0";
    src = fetchurl {
      url = "ftp://public.dhe.ibm.com/storage/tivoli-storage-management/maintenance/client/v7r1/Linux/LinuxX86_DEB/BA/v716/7.1.6.0-TIV-TSMBAC-LinuxX86_DEB.tar";
      sha256 = "1by9cq35jvwpw5lk6yl3inzy4sgr1ibsdw7ycv6298n8vkp92b8j";
    };
    inherit meta;
    ...
    preFixup = ''
      ...
      do
        ...
        if [ "$(cut -b -6 <<< "$target")" != "../../" ]; then
          new_target="$out$target"
        else
          new_target="$out/$(cut -b 7- <<< "$target")"
        fi
        echo "linking: $link -> $new_target"
        ln --symbolic --force --no-target-directory "$new_target" "$link"
      done
    '';
  };
